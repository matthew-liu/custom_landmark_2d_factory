...
class Matcher {
	...
	public:
		int counter; 		   // max #times allowed for resizing
		...
		float match_limit;     // the threshold for ALL acceptable matching points
  		...
  		bool scale_match(cv::Mat& scene, const cv::Mat& templ, 
 	...
 	private:
  		...
  		double match(cv::Mat& scene, const cv::Mat& templ, std::list<cv::Point>& matching);
